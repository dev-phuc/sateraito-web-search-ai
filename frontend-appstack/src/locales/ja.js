const JAPANESE_TRANSLATIONS = {
  translation: {
    // =================== General App Information ===================
    TXT_APP_DESCRIPTION: "AIを活用したウェブ検索体験を提供します",
    TXT_APP_NAME: "ウェブ検索AI",
    TXT_COMPANY_NAME: "サテライト",
    TXT_LOGIN_WITH_GOOGLE: "Googleでログイン",

    // =================== Server Error Messages ===================
    box_search_invalid_config: "ボックス検索の設定が無効です",
    client_website_not_found: "クライアントウェブサイトが見つかりません",
    domain_already_exists: "ドメインは既に存在します",
    domain_is_required: "ドメインは必須です",
    id_is_required: "IDは必須です",
    internal_server_error: "内部サーバーエラー",
    page_url_is_required: "ページURLは必須です",
    invalid_tel_no: "無効な電話番号形式です",
    invalid_mail_address: "無効なメール形式です",
    invalid_content_type: "無効なコンテンツタイプです",
    forbidden: "このリソースにアクセスする権限がありません",
    namespace_error: "名前空間エラーが発生しました。サポートにお問い合わせください",

    // =================== Common UI Elements ===================
    TXT_APP_ID: "アプリID",
    TXT_LOADING: "読み込み中",
    TXT_NOT_SET: "設定されていません",
    TXT_OPTION_ALL: "すべて",
    TXT_REFRESH: "更新",
    TXT_REFRESHING: "更新中...",
    TXT_RELOAD: "再読み込み",
    TXT_SELECT_ALL: "すべて選択",
    TXT_TOTAL_SELECTED: "選択済み",
    TOGGLE_NAVIGATION: "ナビゲーション切替",

    // =================== Button Labels ===================
    BTN_CANCEL: "キャンセル",
    BTN_CLEAR: "クリア",
    BTN_CREATE: "作成",
    BTN_DELETE: "削除",
    BTN_DETAIL: "詳細",
    BTN_EDIT: "編集",
    BTN_FILTER: "フィルター",
    BTN_NEXT_PAGE: "次のページ",
    BTN_NO: "いいえ",
    BTN_PREVIOUS: "前へ",
    BTN_RESET: "リセット",
    BTN_RESET_TO_DEFAULTS: "デフォルトにリセット",
    BTN_SAVE: "保存",
    BTN_SUBMIT: "送信",
    BTN_UPDATE: "更新",
    BTN_YES: "はい",

    // =================== Status and Labels ===================
    LABEL_CREATED_DATE: "作成日",
    LABEL_ERROR_MESSAGE: "エラーメッセージ",
    LABEL_PROMPT: "プロンプト",
    LABEL_RESOURCE: "リソース",
    LABEL_RESPONSE: "応答",
    LABEL_STATUS: "ステータス",
    LABEL_UPDATED_DATE: "更新日",
    STATUS_ACTIVE: "有効",
    STATUS_COMPLETED: "成功",
    STATUS_DISABLED: "無効",
    STATUS_FAILED: "失敗",
    STATUS_OVER_QUOTA: "上限超過",

    // =================== Navigation Menu ===================
    TXT_AI_CONFIGURATION: "AI設定",
    TXT_CLIENT_WEBSITE_USAGE_STATISTICS: "クライアントウェブサイト利用統計",
    TXT_DASHBOARD: "ダッシュボード",
    TXT_DESIGN_BANNER_KEYWORDS: "バナー/キーワードデザイン",
    TXT_DESIGN_SEARCH_BOX: "検索ボックスデザイン",
    TXT_DOMAINS_MANAGEMENT: "ドメイン管理",
    TXT_ENCODER_HTML_JS: "エンコーダーHTML/JS",
    TXT_OPERATIONS_LOGS: "操作ログ",
    TXT_USAGE_STATISTICS: "利用統計",

    // =================== Theme Settings ===================
    BG_COLOR_SCHEME_LABEL: "背景色スキーム",
    COLOR_SCHEME_LABEL: "カラースキーム",
    LAYOUT_BUILDER_DES: "検索エンジンの外観をウェブサイトのデザインに合わせてカスタマイズします。",
    LAYOUT_BUILDER_LABEL: "レイアウトビルダー",
    LAYOUT_LABEL: "レイアウト",
    MENU_BEHAVIOR_LABEL: "メニュー動作",
    MENU_POSITION_LABEL: "メニュー位置",
    TXT_LAYOUT_BOXED: "ボックス",
    TXT_LAYOUT_FLUID: "フルイド",
    TXT_LAYOUT_MATERIAL: "マテリアル",
    TXT_SIDEBAR_BEHAVIOR_COMPACT: "コンパクト",
    TXT_SIDEBAR_BEHAVIOR_FIXED: "固定",
    TXT_SIDEBAR_BEHAVIOR_STICKY: "スティッキー",
    TXT_SIDEBAR_LEFT: "左",
    TXT_SIDEBAR_RIGHT: "右",
    TXT_THEME_COLORED: "カラー",
    TXT_THEME_DARK: "ダーク",
    TXT_THEME_DEFAULT: "デフォルト",
    TXT_THEME_LIGHT: "ライト",

    // =================== Authentication ===================
    PAGE_TITLE_LOGIN: "サインイン",
    TXT_LOGIN_FAILED_PLEASE_TRY_AGAIN: "ログインに失敗しました。もう一度お試しください。",
    TXT_LOGOUT: "ログアウト",

    // =================== 404 Page ===================
    BTN_GO_BACK: "戻る",
    BTN_GO_HOME: "ホームページへ",
    DESC_PAGE_404: "お探しのページは削除されたか、名前が変更されたか、一時的に利用できない可能性があります。",
    TITLE_PAGE_404: "ページが見つかりません",

    // =================== Dashboard ===================
    APP_ID: "アプリID",
    AVAILABLE_START_DATE: "利用開始日",
    BTN_GO_TO_MANAGER_CLIENT_WEBSITES: "クライアントウェブサイト管理へ",
    BTN_MORE_DETAIL_OPERATION_LOGS: "操作ログの詳細",
    BTN_MORE_DETAIL_USAGE_STATISTICS: "利用統計の詳細",
    CANCEL_DATE: "キャンセル日",
    CHARGE_START_DATE: "課金開始日",
    CONTRACT_INFORMATION: "契約情報",
    CONTRACT_STATUS: "契約ステータス",
    CSV_FILE_ENCODING: "CSVファイルエンコーディング",
    LANGUAGE: "言語",
    MAIL_ADDRESS: "メールアドレス",
    MSG_ACTIVE_UNTIL: "契約期間中",
    MSG_INACTIVE_SINCE: "契約期間外",
    OTHER_SETTING: "その他の設定",
    PAGE_TITLE_DASHBOARD_MANAGER: "ダッシュボード",
    PERIOD_OF_CONTRACT: "契約期間",
    PLEASE_CONTACT_SATERAITO_IF_YOU_WANT_TO_USE_SUBDOMAIN_AS_WELL: "※サブドメインも本ドメインに統合して使いたい場合は、サテライトオフィスまでご連絡ください！",
    PLEASE_CONTACT_SATERAITO_TO_CONTINUE_AFTER_TRIAL: "※トライアル期間終了後もご利用を継続される場合は有償のご契約が必要です。サテライトオフィスまでご連絡ください。",
    TARGET_LINK_DOMAINS: "ターゲットリンクドメイン",
    TEL_NO: "電話番号",
    TENANT: "テナント",
    TENANT_INFORMATION: "テナント情報",
    TIMEZONE: "タイムゾーン",

    // =================== Client Websites Management ===================
    BTN_ADD_DOMAIN: "ドメイン追加",
    CONFIRM_DELETE_CLIENT_WEBSITE: "このクライアントウェブサイトを削除してもよろしいですか？",
    LABEL_AI_ENABLED: "AI有効",
    LABEL_DESCRIPTION: "説明",
    LABEL_DOMAIN: "ドメイン",
    LABEL_FAVICON_URL: "ファビコンURL",
    LABEL_SITE_NAME: "サイト名",
    MSG_DATA_CLIENT_WEBSITES_LOADING: "クライアントウェブサイトのデータを読み込み中...",
    MSG_ERROR_DOMAIN_INVALID: "ドメイン形式が無効です（例：example.com）",
    MSG_ERROR_DOMAIN_REQUIRED: "ドメインは必須です！",
    MSG_ERROR_STATUS_REQUIRED: "ステータスは必須です！",
    MSG_PLEASE_ADD_FIRST_CLIENT_WEBSITE: "最初のクライアントウェブサイトを追加してください。",
    MSG_WARNING_FETCH_PAGE_INFO: "警告：ページ情報の取得に失敗しました。ドメインが到達不能の可能性があります。ドメインが正しい場合は、詳細を手動で入力してください。",
    NAME_COL_ACTIONS: "操作",
    NAME_COL_CREATED_DATE: "作成日",
    NAME_COL_STATUS: "ステータス",
    NAME_COL_UPDATED_DATE: "更新日",
    NAME_COL_WEBSITE_AI_ENABLED: "AI有効",
    NAME_COL_WEBSITE_DESCRIPTION: "説明",
    NAME_COL_WEBSITE_NAME: "ウェブサイト名",
    NAME_COL_WEBSITE_URL: "ウェブサイトURL",
    NO_CLIENT_WEBSITES_FOUND: "クライアントウェブサイトが見つかりません。",
    PAGE_TITLE_CLIENT_WEBSITES: "クライアントウェブサイト",
    PLACEHOLDER_CLIENT_WEBSITES_DESCRIPTION: "説明を入力",
    PLACEHOLDER_CLIENT_WEBSITES_DOMAIN: "ドメインを入力（例：example.com）",
    PLACEHOLDER_CLIENT_WEBSITES_FAVICON_URL: "ファビコンURLを入力（例：https://example.com/favicon.ico）",
    PLACEHOLDER_CLIENT_WEBSITES_SITE_NAME: "サイト名を入力",
    TITLE_ADD_DOMAIN: "ドメイン追加",
    TXT_FETCHING_PAGE_INFO: "ページ情報を取得中...",
    TITLE_CLIENT_WEBSITES_MANAGEMENT: "クライアントウェブサイト管理",
    TITLE_CONFIRM_DELETE_CLIENT_WEBSITE: "クライアントウェブサイト削除",
    TXT_CREATE_CLIENT_WEBSITES_SUCCESS: "クライアントウェブサイトが正常に作成されました！",
    TXT_DELETE_CLIENT_WEBSITES_SUCCESS: "クライアントウェブサイトが正常に削除されました！",
    TXT_ERROR_CREATE_CLIENT_WEBSITES: "クライアントウェブサイトの作成に失敗しました。もう一度お試しください。",
    TXT_ERROR_DELETE_CLIENT_WEBSITES: "クライアントウェブサイトの削除に失敗しました。もう一度お試しください。",
    TXT_ERROR_UPDATE_CLIENT_WEBSITES: "クライアントウェブサイトの更新に失敗しました。もう一度お試しください。",
    TXT_FETCH_CLIENT_WEBSITES_ERROR: "クライアントウェブサイトの取得に失敗しました。もう一度お試しください。",
    TXT_SELECT_AT_LEAST_ONE_ITEM: "少なくとも1つの項目を選択してください。",
    TXT_UPDATE_CLIENT_WEBSITES_SUCCESS: "クライアントウェブサイトが正常に更新されました！",

    // =================== Box Search Configuration ===================
    BTN_COPY_CODE: "コードをコピー",
    BUTTON_COPY_CODE: "コードをコピー",
    LABEL_BACKGROUND_COLOR: "背景色",
    LABEL_BORDER_RADIUS: "角丸",
    LABEL_BOX: "ボックス",
    LABEL_BUTTON_BACKGROUND_COLOR: "ボタン背景色",
    LABEL_BUTTON_BORDER_RADIUS: "ボタン角丸",
    LABEL_COLOR: "色",
    LABEL_FONT: "フォント",
    LABEL_FONT_SIZE: "フォントサイズ",
    LABEL_FULLSCREEN: "全画面",
    LABEL_FULLSCREEN_BLUR: "全画面ぼかし",
    LABEL_ICON: "アイコン",
    LABEL_OPTIONS: "オプション",
    LABEL_PADDING: "パディング",
    LABEL_PREVIEW: "ライブプレビュー",
    LABEL_SEARCH_BOX: "検索ボックス",
    LABEL_SEARCH_BUTTON: "検索ボタン",
    LABEL_SHADOW: "シャドウ",
    LABEL_SHOW_BACKGROUND: "背景を表示",
    LABEL_SHOW_BOX_SEARCH: "検索ボックスを表示",
    LABEL_SHOW_HTML: "ソースコード",
    LABEL_THEME: "テーマ",
    LABEL_TYPE: "タイプ",
    MODAL_TEXT_CONFIRM_RESET_BOX_SEARCH_CONFIG_TO_DEFAULTS: "このアクションはボックス検索設定をデフォルト設定にリセットします。このアクションは元に戻すことができません。本当に続行しますか？",
    NOTICE_COPIED_TO_CLIPBOARD: "クリップボードにコピーされました",
    PAGE_TITLE_BOX_SEARCH_CONFIG: "ボックス検索設定",
    PLACEHOLDER_SEARCH: "何でも検索...",
    TXT_ADJUST_FONT_SIZE: "フォントサイズを調整します。",
    TXT_BUTTON_CORNER_RADIUS: "ボタンの角丸ピクセル数。",
    TXT_CHOOSE_SEARCH_UI: "検索UIの表示方法を選択してください。",
    TXT_CORNER_RADIUS: "角丸のピクセル数。",
    TXT_ENABLE_SHADOW: "ボックスにシャドウを追加します。",
    TXT_ERROR_RESET_BOX_SEARCH_CONFIG: "ボックス検索設定のリセットに失敗しました。もう一度お試しください。",
    TXT_ERROR_UPDATE_BOX_SEARCH_CONFIG: "ボックス検索設定の更新に失敗しました。もう一度お試しください。",
    TXT_FONT_DESC: "カンマ区切りのフォントファミリーのフォールバックリスト。",
    TXT_FONT_PLACEHOLDER: "例：Inter、Arial、system-ui",
    TXT_FORM_SUBMIT_CONDITION: "変更があり、バリデーションが通った場合のみフォームが送信されます。",
    TXT_ICON_DESC: "Material Iconsなどのアイコンクラス名やURL。",
    TXT_ICON_PLACEHOLDER: "例：search、magnifier、またはSVGクラス",
    TXT_INNER_SPACING: "検索ボックス内の余白。",
    TXT_PICK_BACKGROUND_COLOR: "検索ボックスの背景色を選択してください。",
    TXT_PREVIEW_DESC: "検索ボックスの外観と操作感を確認できます",
    TXT_RESET_BOX_SEARCH_CONFIG_SUCCESS: "ボックス検索設定がデフォルトに正常にリセットされました！",
    TXT_SHOW_HTML_DESC: "ウェブサイトに検索ボックスを埋め込むための生成されたHTML/JSコードスニペットを表示します。",
    TXT_UPDATE_BOX_SEARCH_CONFIG_SUCCESS: "ボックス検索設定が正常に更新されました！",

    // =================== LLM Configuration ===================
    LABEL_LLM_CONFIGURATION: "LLM設定",
    LABEL_MODEL_NAME: "モデル名",
    LABEL_RESPONSE_HIGH: "高い",
    LABEL_RESPONSE_LENGTH_LEVEL: "応答長レベル",
    LABEL_RESPONSE_MEDIUM: "中",
    LABEL_RESPONSE_LOW: "低い",
    LABEL_SYSTEM_PROMPT: "システムプロンプト",
    LABEL_DOMAIN_FILTER_SETTINGS: "ドメインフィルター設定",
    LABEL_ENABLED_DOMAIN_FILTER: "ドメインフィルタリングを有効にする",
    LABEL_SEARCH_DOMAIN_FILTER: "許可ドメイン",
    LABEL_EXCLUDED_DOMAIN_FILTER: "ブロックドメイン",
    LABEL_ALLOWED_DOMAINS: "許可ドメイン ({{count}})",
    LABEL_BLOCKED_DOMAINS: "ブロックドメイン ({{count}})",
    MODAL_TEXT_CONFIRM_RESET_TO_DEFAULTS: "このアクションはLLM設定をデフォルト設定にリセットします。このアクションは元に戻すことができません。本当に続行しますか？",
    MODAL_TITLE_CONFIRM_RESET_TO_DEFAULTS: "デフォルトへのリセットを確認",
    MSG_ERROR_MODEL_NAME_REQUIRED: "モデル名は必須です！",
    MSG_ERROR_RESPONSE_LENGTH_LEVEL_REQUIRED: "応答長レベルは必須です！",
    PAGE_TITLE_LLM_CONFIGURATION: "LLM設定",
    TXT_DOMAIN_FILTER_DESCRIPTION: "有効にすると、AIは許可されたドメイン内でのみ検索し、ブロックされたドメインを検索結果から除外します。",
    TXT_SEARCH_DOMAIN_PLACEHOLDER: "ドメインを入力 (例: example.com)",
    TXT_EXCLUDED_DOMAIN_PLACEHOLDER: "除外するドメインを入力 (例: example.com)",
    TXT_SEARCH_DOMAIN_HELP: "AIが情報を検索することを許可するドメインを追加してください。Enterキーを押すか+をクリックして追加します。",
    TXT_EXCLUDED_DOMAIN_HELP: "検索結果から除外すべきドメインを追加してください。Enterキーを押すか+をクリックして追加します。",
    TXT_NO_DOMAINS_ADDED: "まだドメインが追加されていません",
    MSG_WARNING_DOMAIN_ALREADY_EXISTS: "このドメインは既に追加されています",
    MSG_SUCCESS_DOMAIN_ADDED: "ドメインが正常に追加されました",
    MSG_INFO_DOMAIN_REMOVED: "ドメインが削除されました",
    TXT_ERROR_RESET_LLM_CONFIGURATION: "LLM設定のリセットに失敗しました。もう一度お試しください。",
    TXT_ERROR_UPDATE_LLM_CONFIGURATION: "LLM設定の更新に失敗しました。もう一度お試しください。",
    TXT_FETCH_LLM_CONFIGURATION_ERROR: "LLM設定の取得に失敗しました。もう一度お試しください。",
    TXT_MODEL_NAME_PLACEHOLDER: "モデル名は読み取り専用で変更できません。",
    TXT_RESET_LLM_CONFIGURATION_SUCCESS: "LLM設定がデフォルトに正常にリセットされました！",
    TXT_SYSTEM_PROMPT_PLACEHOLDER: "システムプロンプトを入力",
    TXT_UPDATE_LLM_CONFIGURATION_SUCCESS: "LLM設定が正常に更新されました！",

    // =================== Operation Logs ===================
    LABEL_FROM_DATE: "開始日",
    LABEL_TO_DATE: "終了日",
    MSG_DATA_OPERATION_LOGS_LOADING: "操作ログデータを読み込み中...",
    MSG_ERROR_CLIENT_DOMAIN_REQUIRED: "クライアントドメインが必要です！",
    MSG_ERROR_FROM_DATE_REQUIRED: "開始日が必要です！",
    MSG_ERROR_TO_DATE_REQUIRED: "終了日が必要です！",
    NAME_COL_APP_ID: "アプリID",
    NAME_COL_CLIENT_DOMAIN: "クライアントドメイン",
    NAME_COL_CREATED_AT: "作成日時",
    NAME_COL_MODEL_NAME: "モデル名",
    NAME_COL_PROMPT: "プロンプト",
    NAME_COL_STATUS: "ステータス",
    NAME_COL_TENANT: "テナント",
    NO_OPERATION_LOGS_AVAILABLE: "操作ログがありません",
    PAGE_TITLE_OPERATION_LOGS: "操作ログ",
    PLACEHOLDER_SELECT_CLIENT_WEBSITE: "クライアントウェブサイトを選択",
    TITLE_DETAIL_OPERATION_LOG: "操作ログの詳細",
    TXT_NO_OPERATION_LOGS_AVAILABLE_DESC: "選択したフィルターに操作ログが見つかりません。",
    TXT_SHOWING_OPERATION_LOGS: "{{total}}件の操作ログのうち{{count}}件を表示",

    // =================== LLM Usage ===================
    LABEL_CHART_TYPE: "チャートタイプ",
    LABEL_COMPLETION_LENGTH: "完了の長さ",
    LABEL_CURRENT_MONTH_REQUESTS: "今月のリクエスト数",
    LABEL_LAST_MONTH_REQUESTS: "先月のリクエスト数",
    LABEL_LLM_USAGE_BREAKDOWN: "LLM使用状況の内訳",
    LABEL_PROMPT_LENGTH: "プロンプトの長さ",
    LABEL_TIME_FRAME: "時間枠",
    LABEL_TIMESTAMP: "タイムスタンプ",
    LABEL_TOTAL_LENGTH: "総長さ",
    LABEL_TOTAL_REQUESTS: "総リクエスト数",
    LAST_RESET: "最終リセット日",
    LLM_QUOTA_MONTHLY: "月間LLMクォータ",
    LLM_QUOTA_MONTHLY_DESC: "現在の月の総LLMクォータ。",
    LLM_QUOTA_REMAINING: "残りLLMクォータ",
    LLM_QUOTA_REMAINING_DESC: "現在の月にまだ使用可能なLLMクォータ。",
    LLM_QUOTA_USED: "使用済みLLMクォータ",
    LLM_QUOTA_USED_DESC: "現在の月に使用されたLLMクォータ。",
    MSG_DATA_LLM_USAGE_LOADING: "LLM使用データを読み込み中...",
    MSG_DATA_LLM_USAGE_NO_DATA: "この期間のLLM使用データがありません。",
    MSG_LLM_USAGE_BREAKDOWN_INFO: "この表は、時間ごとのLLM使用状況の詳細な内訳を提供します。プロンプトの長さ、完了の長さ、総長さ、およびリクエスト数が含まれます。",
    NEXT_RESET: "次のリセット日",
    PAGE_TITLE_LLM_USAGE_MANAGER: "LLM使用状況",
    TXT_ALL_TIME: "全期間",
    TXT_AREA_CHART: "エリアチャート",
    TXT_BAR_CHART: "バー チャート",
    TXT_LAST_MONTH: "先月",
    TXT_LINE_CHART: "ラインチャート",
    TXT_THIS_MONTH: "今月",
    TXT_THIS_WEEK: "今週",
    TXT_TODAY: "今日",

    // =================== Other Settings ===================
    LANGUAGE_CONFIG: "言語設定",
    LAYOUT_ANIMATION_LABEL: "レイアウトとアニメーション",
    MSG_UPDATE_USER_CONFIG_FAILED: "ユーザー設定の更新に失敗しました。もう一度お試しください。",
    MSG_UPDATE_USER_CONFIG_SUCCESS: "ユーザー設定が正常に更新されました！",
    PAGE_TITLE_OTHER_SETTINGS: "その他の設定",
    TXT_SETTINGS: "設定",

    // =================== Contract Information ===================
    EDIT_CONTRACT_INFORMATION: "契約情報編集",
    MSG_ERROR_INVALID_EMAIL: "無効なメール形式です！",
    MSG_ERROR_REQUIRED: "必須項目です！",
    LABEL_MAIL_ADDRESS: "メールアドレス",
    PLACEHOLDER_MAIL_ADDRESS: "メールアドレスを入力",
    LABEL_TEL_NO: "電話番号",
    PLACEHOLDER_TEL_NO: "電話番号を入力（例：+81-3-1234-5678）",
    TITLE_CONTRACT_INFORMATION: "契約情報",
    TXT_UPDATE_SUCCESS: "契約情報が正常に更新されました！",
    TXT_ERROR_UPDATE_FAILED: "契約情報の更新に失敗しました。もう一度お試しください。",

    // Other translations can be added here
  }
};

export default JAPANESE_TRANSLATIONS;

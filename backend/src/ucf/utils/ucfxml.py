...
#		return UcfUtil.nvl(UcfUtil.getHashStr(hash_node, xpath))
	...
	def getAttribute(self, name):
		...
		RETURN:属性値
		'''
		# if self._element.attrib.has_key(name):  # g2対応
	...
	def exchangeToHash(self, isAttr=False, isChild=False, prefix=''):
		...
		for node in self._element:
			...
			# if not dic.has_key(dic_key):  # g2対応
			if not dic_key in dic:
				dic[dic_key]=value
	...
	def outerXml(self):
		...
		'''
		from xml.etree import ElementTree
		# return unicode(ElementTree.tostring(self._element,'utf-8'), 'utf-8')  # g2対応
		return str(ElementTree.tostring(self._element, 'utf-8'), 'utf-8')
	#+++++++++++++++++++++++++++++++++++++++

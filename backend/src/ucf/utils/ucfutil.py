...
class UcfUtil():
	...
	def reFormatDate(dateString, frChar, toChar):
		return dateString.replace(frChar, toChar)

	reFormatDate = staticmethod(reFormatDate)
	...
	def deflateCompress(value):
		logging.debug('==========deflateCompress===========')
		# deflated_request = zlib.compress(value)[2:-4]
		# return base64.b64encode(zlib.compress(str(value))[2:-4])
		return UcfUtil.b64encode(zlib.compress(compat.to_bytes(value))[2:-4])
...
#			return '';
	...
	# get value only one
	def editInsertTagOnly(target, data, start_tag, end_tag):
		u''' データを差し込む(mailutilから移動) 2011/05/18 '''
		...
		if start_idx >= 0 and end_idx >= 0 and start_idx < end_idx:
			...
			result = UcfUtil.getHashStr(data, key)

		return result

	editInsertTagOnly = staticmethod(editInsertTagOnly)
	...
	escape_url = staticmethod(escape_url)
	...
	def getValueInDict(dict, name):
		if (name in dict):
			return dict[name]
		return None
	getValueInDict = staticmethod(getValueInDict)

	def getValueBoolInDict(dict, name):
		if (name in dict):
			return dict[name]
		return False
	getValueBoolInDict = staticmethod(getValueBoolInDict)

	def getValueIntInDict(dict, name):
		if (name in dict):
			return dict[name]
		return 0
	getValueIntInDict = staticmethod(getValueIntInDict)

	def decodeStringAscii(strVal):
		if isinstance(strVal ,bytes):
			return strVal.decode('ascii')
		return strVal
	decodeStringAscii = staticmethod(decodeStringAscii)

	def decodeString(strVal,encode='utf-8'):
		if isinstance(strVal ,bytes):
			return strVal.decode(encode)
		return strVal

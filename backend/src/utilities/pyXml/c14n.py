...
class _implementation:
    ...
    def _do_element(self, node, initial_other_attrs = [], unused = None):
        ...
        logging.debug(initial_other_attrs)
        logging.debug(_attrs(node))
        logging.debug(type(initial_other_attrs))
        logging.debug(type(_attrs(node)))
...
def Canonicalize(node, output=None, **kw):
    ...
    '''
    if output:
        # GAEGEN2対応：Python3ではapply関数は廃止されたので変更

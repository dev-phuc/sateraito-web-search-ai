#!/usr/bin/python
# -*- coding: utf-8 -*-
# Copyright 息2011 Andrew D. Yates
...
"""
import re
import base64    # GAEGEN2対応
import sateraito_logger as logging	# GAEGEN2対応
# GAEGEN2対応
...
RX_PUBLIC_KEY = re.compile("subjectPublicKey=([0-9]+)")
...
def parse(data):
  ...
  """
    ...
    raw_data = base64.b64decode(body.encode())
    ...
    #logging.debug(key_bits)
    #logging.debug(type(key_bits))
    #logging.debug(int(key_bits, 2))
    # GAEGEN2対応：何要因か不明ながら key_bits に2進数ではなく10進数文字列がセットされるようになった（pyasn1のdecoderの使い方？）＆ hexの戻り地が 0x1234L → 0x1234 形式に変わったので対応
    #key_binhex = hex(int(key_bits, 2))[2:-1]
    #key_bin = key_binhex.decode("hex")
    key_binhex = hex(int(key_bits, 10))[2:]
    key_bin = bytes.fromhex(key_binhex)

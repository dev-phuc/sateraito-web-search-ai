...
import sateraito_db
from ucf.utils import ssofunc  # GAEGEN2対応：SAML、SSO連携対応
...
class _OidcCallback(sateraito_page.Handler_Basic_Request, sateraito_page._BasePage):
	...
	def _getSSOPublicKey(self, thumbprint):
		public_key = None
		exponent = None
		modulus = None
			...
			if public_key is not None and exponent is not None and modulus is not None:
				...
				while process_cnt <= MAX_RETRY_CNT:
					try:
						...
						request_url = 'https://sso.sateraito.jp/a/common/discovery/keys'
						deadline = 5
						...
						if result.status_code != 200:
							...
							#keys_dict = json.JSONDecoder().decode(result.content)  # g2対応
	...
	def analysisAndRetrieveIDToken(self, id_token):
		...
		if not sateraito_inc.debug_mode:
			...
			if thumbprint is None or thumbprint == '':
				...
				return None
			public_key, exponent, modulus = self._getSSOPublicKey(thumbprint)
	...
	def _process(self):
		...
		if len(states) >= 3 and states[1] != '':
			...
			for item in infos:
				...
				# if k == 'sso4auth':
				#	sso_for_authorize = v
				if k == 'authkind' and v == 'setup':
					for_setup = True
				if k == 'hl':
					hl = v
		logging.info('with_error_page=' + str(with_error_page))
		logging.info('hl=' + str(hl))
		# logging.info('sso_for_authorize=' + sso_for_authorize)
		...
		if error is not None and error != '':
			if with_error_page:
				...
				if error == 'login_required':
					...
					ret_datas.append(my_lang.getMsg('INVALID_AUTH_OPENID_CONNECT_WITH_SSO') + error)
			...
			# １ページ複数ガジェット対応（いまのところガジェットで使う予定はないけど）
			# check state
			state_from_cookie = self.request.cookies.get('oidc_state', None)
			logging.info('state_from_cookie=' + str(state_from_cookie))
			if state_from_cookie is None or str(state_from_cookie) != str(state):
				...
				if with_error_page:
					...
					ret_datas.append('<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />')
					ret_datas.append('</head><body>')
					ret_datas.append(my_lang.getMsg('INVALID_AUTH_OPENID_CONNECT_WITH_SSO'))
					ret_datas.append('</body></html>')
					return ''.join(ret_datas)
				else:
					self.response.set_status(403)
		...
		if aud != sateraito_inc.SSO_OIDC_CLIENT_ID:
		...
		# is_admin
		is_sso_admin = jwt_data.get('is_admin', False)
		logging.info('is_sso_admin=' + str(is_sso_admin))
		# tenant_or_domain
		...
		if for_setup:
			if not is_sso_admin:
				if with_error_page:
					...
					ret_datas.append('<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />')
					ret_datas.append('</head><body>')
					ret_datas.append(my_lang.getMsg('FAILED_AUTH_BY_PRIVILEGE_ADMIN'))
					ret_datas.append('</body></html>')
					return ''.join(ret_datas)
				else:
					self.response.set_status(403)
			...
			if contract_entry is None or contract_entry.tenant != tenant_or_domain:
				logging.warning('Invalid State.')
				self.response.write('Invalid State.')
				if with_error_page:
					my_lang = sateraito_func.MyLang(hl)
					ret_datas = []
					ret_datas.append('<html><head>')
					ret_datas.append('<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />')
					ret_datas.append('</head><body>')
					ret_datas.append(my_lang.getMsg('FAILED_RETRIEVE_SSO_INFO'))
					ret_datas.append('</body></html>')
					return ''.join(ret_datas)
				else:
			...
			# SSOからX.509証明書などを取得してセット（OIDC、OAuth2.0の仕組みにのっとってもいいが...）
			sso_fqdn = sateraito_func.getSSOFQDN(contract_entry.sso_for_authorize)
			sso_api_results = ssofunc.getSSOTenantInfo(sso_fqdn, contract_entry.sso_tenant)
			...
			if sso_api_results.get('sso_public_key', '') == '':
				if with_error_page:
					my_lang = sateraito_func.MyLang(hl)
					ret_datas = []
					ret_datas.append('<html><head>')
					ret_datas.append('<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />')
					ret_datas.append('</head><body>')
					ret_datas.append(my_lang.getMsg('FAILED_RETRIEVE_SSO_PUBLIC_KEY'))
					ret_datas.append('</body></html>')
					return ''.join(ret_datas)
				else:
					self.response.set_status(500)
			...
			# X.509証明書をセット
			contract_entry.sso_public_key = sso_api_results.get('sso_public_key', '')
			contract_entry.put()
		...
		user_token = rc.registOrUpdateUserEntry(tenant_or_domain, user_email, target_domain, user_object_id, opensocial_container, user_object_id, is_sso_admin)
...
# 	(r'/sso/oidccallback', OidcCallback),
# ], debug=sateraito_inc.debug_mode, config=sateraito_page.config))

def add_url_rules(app):
	app.add_url_rule('/sso/oidccallback', view_func=OidcCallback.as_view('OidcCallback4ssoSsoOidcCallback'))

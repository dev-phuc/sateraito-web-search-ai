...
class SSOFunc():
	...
	def validate_sign(xml, cert=None, fingerprint=None, fingerprintalg='sha1', validatecert=False, debug=False):
			...
			try:
					...
					signature_nodes = SSOFunc.query(elem, '/samlp:Response/ds:Signature')

					if not len(signature_nodes) > 0:
							signature_nodes += SSOFunc.query(elem, '/samlp:Response/saml:Assertion/ds:Signature')

					if len(signature_nodes) == 1:
							signature_node = signature_nodes[0]
							return SSOFunc.validate_node_sign(signature_node, elem, cert, fingerprint, fingerprintalg, validatecert, debug)
					else:
							return False
	...
	def validate_node_sign(signature_node, elem, cert=None, fingerprint=None, fingerprintalg='sha1', validatecert=False, debug=False):
			...
			try:
					...
					public_key = utilities.rsa_x509_pem.get_key(public_key_dict)
					f_public = utilities.rsa_x509_pem.f_public(public_key)
					...
					#logging.info(reference_uri)
					# 署名が付いているXMLノードを取得
					reference_id = ''
					if reference_uri.startswith('#'):
						reference_id = reference_uri.lstrip('#')
					#logging.info('reference_id=' + reference_id)
					if reference_id != '':
						...
						if nodelist is not None and len(nodelist) > 0:
							...
							output = io.BytesIO()
							etree.ElementTree(nodelist[0]).write_c14n(output, exclusive=0, with_comments=0)
						...
						#unsigned_xml = c14n.Canonicalize(unsigned_xml)
						output = io.BytesIO()
						etree.ElementTree(elem).write_c14n(output, exclusive=0, with_comments=0)
						unsigned_xml = output.getvalue().decode()
					
					# LINE WORKS直接認証対応…雑だが正規化方式をSAMLResponseを見て判断する 2019.06.13
					with_comments = unsigned_xml.find('#WithComments') >= 0
					with_c14n = unsigned_xml.find('xml-exc-c14n') >= 0
					logging.info('with_comments=%s with_c14n=%s key_size=%s' % (with_comments, with_c14n, key_size))
					...
					#return utilities.xmldsig.verify(unsigned_xml, key, f_public, key_size, with_c14n=with_c14n, with_comments=with_comments, reference_uri=reference_uri)
...
class SamlAuthnRequest():
	...
	def __init__(self, samlsettings, force_authn=False, is_passive=False, set_nameid_policy=True):
			...
			self.__id = uid
			issue_instant = SSOFunc.parse_time_to_SAML(datetime.datetime.now())
	...
	def redirect_to_idp(self, page, relay_state=None):
		...
		#query_relay_state = page.request.url
...
class SamlResponse():
	...
	def __init__(self, samlsettings, response):
			...
			self.__error = None
			# LINE WORKS直接認証対応…Deflate圧縮パターンに対応 2019.06.13
			try:
	...
	def is_valid(self, page, request_id=None, use_cert_in_samlresponse=False):
			...
			try:
					...
					# LINE WORKS直接認証対応…SAMLResponseの署名からX.509公開証明書を取得 2019.06.13
...
#			return nameid_value
	...
	def get_nameid(self):
			nameid = ''
			# LINE WORKS直接認証対応…本当はもっとスマートに対応できるはずだが取り急ぎ... 2019.06.13
			#nameid_nodes = self.__query_assertion('/saml:Subject/saml:NameID')
			...
			## LINE WORKS直接認証対応…本当はもっとスマートに対応できるはずだが取り急ぎ個別対応 2019.06.13
	...
	def get_familyname(self):
			...
			if familyname != '':
				...
				familyname = base64.decodebytes(familyname.encode()).decode()
	...
	def get_givenname(self):
			...
			if givenname != '':
				...
				givenname = base64.decodebytes(givenname.encode()).decode()
